...
set.seed(10)
plsr_cv_out <- plsr(Balance ~ ., data = train, validation = "CV")
...
y_test = test[,12]
plsr_pred = predict(plsr_cv_out, x_test)
plsr_test_mse = mean((plsr_pred - y_test)^2)

# full model
plsr_full <- plsr(Balance ~., data = scaled_credit, ncomp = plsr_ncomp)

# coefficients of plsr model
plsr_coef <- c(0,as.vector(coef(plsr_full)))
